...
import { PiPlusBold, PiMinusBold, PiStarFill, PiStar, PiTrashSimpleBold } from "react-icons/pi";
import img1 from "../images/art1.jpg";
import { IoBagAddOutline, IoBagRemove } from "react-icons/io5";
...
const ProductItem: React.FC<ProductItemProps> = ({id, title, heightCm, widthCm, isFeatured, showButton, price, artworkId, media }) => {
  ...
  return (
    <div className=" border rounded-sm bg-white overflow-hidden">
      ...
      <section className=" border-t px-2 py-2">
        ...
        {showButton && <div className=" flex items-center gap-3 mb-1">
        ...
        <div className="flex items-center justify-between">
          ...
          {showButton &&<div className=" ">
            {!(existing || existingArtwork) && (
              <div onClick={addHandler} className=" flex gap-1 items-center justify-center bg-blue-600 hover:bg-blue-700 text-xs font-semibold text-white px-3 py-1 rounded-sm cursor-pointer shadow-sm hover:shadow-md">
                <PlusIcon/> <span>Cart</span>
              </div>
            )}
            {(existing) && (
              <div className=" flex justify-between w-20">
              <button onClick={() =>decreaseQuantity(id)} className=" bg-blue-600 hover:bg-blue-700 text-xs text-white px-2 rounded-sm shadow-sm hover:shadow-md">
                {existing.quantity < 2 ? <DeleteIcon/> : <MinusIcon/>}
              </button>
              <p className=" text-gray-600">{existing && existing.quantity}</p>
              <button onClick={() =>increaseQuantity(id)} className=" bg-blue-600 hover:bg-blue-700 text-xs font-semibold text-white px-2 rounded-sm shadow-sm hover:shadow-md">
                <PlusIcon/>
              </button>
            </div>
            )}
